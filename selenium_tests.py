...
def test_incorrect_login():
    ...
    browser.find_element(By.ID, "loginform-login").send_keys("8200446")
    browser.find_element(By.ID, "loginform-password").send_keys(
        os.environ["orioks_password"] + "123" + Keys.RETURN
    )
    try:
        alert = (
...
def test_login():
    ...
    browser.find_element(By.ID, "loginform-login").send_keys("8200446")
    browser.find_element(By.ID, "loginform-password").send_keys(
        os.environ["orioks_password"] + Keys.RETURN
    )
    time.sleep(3)
    assert "ОРИОКС" in browser.title
    time.sleep(1)
