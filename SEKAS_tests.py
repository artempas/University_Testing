...
def test_solve_task_incorrectly():
    ...
    assert browser.find_element(By.CLASS_NAME, 'text_ex').find_element(By.TAG_NAME,
                                                                       'h1').text == 'Напишите функцию, которая на вход получает число х, а возвращает х+1'
...
def f(x):
    ...
    assert 'page-num-fail' in browser.find_element(value='links-1').find_element(By.TAG_NAME,'li').get_attribute('class')
...
def f(x):
    ...
    assert 'page-num-success' in browser.find_element(value='links-1').find_element(By.TAG_NAME,'li').get_attribute('class')


def test_register_existing_user():
    browser.delete_all_cookies()
    browser.get(r'https://aekas.newacidrain.repl.co/register')
    browser.find_element(By.ID, 'id_username').send_keys('test_user'+Keys.TAB+'ungabunga123'+Keys.TAB+'ungabunga123')
...
def test_login_and_invalid_login():
    ...
    assert test == "Образовательная платформа НАЗВАНИЕ"
...
def test_ex_text():
        ...
        assert test == "Напишите функцию, которая на вход получает число х, а возвращает х+1"
...
def test_reg():
        ...
        browser.get(url=URL)
        browser.find_element(By.NAME, value="username").send_keys('acid12345')
        time.sleep(2)
